...
<h2 style="color: whitesmoke">Functional Reproducibility</h2>

<span style="font-weight: lighter">Engineering reproducibility in the face of entropy</span>
...
::: notes
In order to use energy we need order in the universe. An ordered division between hot places and cold ones, as depicted on the right.

Once we've made use of the difference the heat evens out and now both places are merely warm, as depicted on the left. We can no longer use that energy. Order has given way to disorder.
...
## Engineering reproducibility in the face of entropy { .center }
...
You should certainly use a version control tool like `git` to manage your code and models but line-by-line diffs and patches would be very inefficient way to store data versions. For data, the parallel here isn't to source control but to software releases.
...
::: notes
Just because you can re-run your pipeline locally, it doesn't prove that it's reproducible. It's not demonstrably reproducible until it's running on emphemeral resources in a build system.
...
::: notes
Once you have an automated system you can develop it continuously.
...
I should start by saying that exploratory data analysis doesn't need to be reproducible. We should have the freedom to explore the data interactively to build up our understanding. We should expect to encounter dead-ends. We should be prepared to throw away code.
:::
...
::: notes
This is where REPL-driven development shines. In RStudio, Emacs, or VS Code you have two windows: one to edit code (here on the left) and the other to run it (on the right). You send code from one to the other, in a read-evaluate-print-loop process. You can tweak code in the REPL console until you've got it right. Once you find something you want to use, share or just keep for future reference you can copy it back up to the editor. This let's us iterate quickly in a carefree, non-reproducible way before finding something worth making reproducible. We can explore methods without needing to worry about architecture.
